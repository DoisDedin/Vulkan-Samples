...
#include "interactive_square_01.h"
...
namespace
{
const std::array<InteractiveSquare01::Vertex, 4> kSquare = {{
    {{-0.5f, -0.5f}, {0.0f, 0.6f, 0.0f}},
    {{-0.5f, 0.5f}, {0.0f, 0.8f, 0.0f}},
    {{0.5f, -0.5f}, {0.0f, 0.7f, 0.3f}},
    {{0.5f, 0.5f}, {0.0f, 0.9f, 0.4f}},
}};
}        // namespace

InteractiveSquare01::InteractiveSquare01()
{
	title = "Interactive Square 01";
	default_clear_color = {0.02f, 0.02f, 0.02f, 1.0f};
}
...
bool InteractiveSquare01::prepare(const vkb::ApplicationOptions &options)
{
...
void InteractiveSquare01::create_vertex_buffer()
{
	const VkDeviceSize buffer_size = kSquare.size() * sizeof(Vertex);
	vertex_buffer                  = std::make_unique<vkb::core::BufferC>(
        get_device(),
        buffer_size,
        VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
        VMA_MEMORY_USAGE_CPU_TO_GPU);
	vertex_buffer->update(kSquare.data(), buffer_size);
}

void InteractiveSquare01::create_uniform_buffer()
{
...
void InteractiveSquare01::update_uniform_buffer()
{
	const float aspect = (height == 0) ? 1.0f : static_cast<float>(width) / static_cast<float>(height);
	const float ortho_height = 1.5f;
	glm::mat4 projection = glm::ortho(-aspect * ortho_height, aspect * ortho_height, -ortho_height, ortho_height, -1.0f, 1.0f);
	glm::mat4 model      = glm::rotate(glm::mat4(1.0f), glm::radians(rotation_degrees), glm::vec3(0.0f, 0.0f, 1.0f));

	UniformBufferObject ubo{};
	ubo.mvp = projection * model;
	uniform_buffer->update(&ubo, sizeof(ubo));
}

void InteractiveSquare01::setup_descriptor_pool()
{
...
void InteractiveSquare01::setup_descriptor_set_layout()
{
...
void InteractiveSquare01::setup_descriptor_set()
{
...
void InteractiveSquare01::prepare_pipeline()
{
	...
	shader_stages[0] = load_shader("interactive_square_01", "interactive.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
	shader_stages[1] = load_shader("interactive_square_01", "interactive.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
...
void InteractiveSquare01::build_command_buffers()
{
	...
	for (size_t i = 0; i < draw_cmd_buffers.size(); ++i)
	{
		...
		vkCmdDraw(draw_cmd_buffers[i], static_cast<uint32_t>(kSquare.size()), 1, 0, 0);
...
void InteractiveSquare01::render(float delta_time)
{
	if (!prepared)
	{
		return;
...
void InteractiveSquare01::input_event(const vkb::InputEvent &input_event)
{
	...
	const auto &key = static_cast<const vkb::KeyInputEvent &>(input_event);
	if (key.get_action() != vkb::KeyAction::Down)
	{
		return;
	}

	if (key.get_code() == vkb::KeyCode::R)
	{
		background_red        = true;
		default_clear_color   = {0.4f, 0.0f, 0.0f, 1.0f};
		rebuild_command_buffers();
	}
	else if (key.get_code() == vkb::KeyCode::B)
	{
...
bool InteractiveSquare01::resize(uint32_t new_width, uint32_t new_height)
{
...
{
	return std::make_unique<InteractiveSquare01>();
}

void InteractiveSquare01::draw_frame()
{
